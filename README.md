...
- [**Идемпотентность**](#идемпотентность)
    - [Пример холодного старта реестра на **5.000.000** ключей](#пример-холодного-старта-реестра-на-5000000-ключей)
...
Данные реестры реализуют:
- Хранение ключей в оперативной памяти.
...
- Ключи и данные в оперативной памяти занимают места *O(_Число ключей * (sizeof(long) + размер ключа + размер данных в байтах)_)* и не расходуют память на служебюную информацию CLR.
- Параллельная регистрация одного и того же уникального ключа не приводит к задвоениям, поток регистрирующий дубликат получит внятный отказ в регистрации.
...
### Пример холодного старта реестра на **5.000.000** ключей
...
```
Время создания и 100% инициализации реестра : 00:00:01.3087314
Занято памяти : 167 320 408 байт
Количество реальных подключений к БД : 5000087
Количество сессий мапперов : 5000088
Число зарегестрированных ключей : 5000000
