...
- [**Идемпотентность**](#идемпотентность)
    - [Пример холодного старта реестра на *10.000.000* ключей](#пример-холодного-старта-реестра-на-10000000-ключей)
...
- Ключи и данные в оперативной памяти занимают места *O(_Число ключей * (9 + размер ключа + размер данных)_)* и не расходуют память на служебную информацию CLR.
...
### Пример холодного старта реестра на **10.000.000** ключей
...
```
Время создания и 100% инициализации реестра : 00:00:02.3911098
Занято памяти : 333 912 120 байт
Количество реальных подключений к БД : 1
Количество сессий мапперов : 1
Число зарегестрированных ключей : 10 000 000
